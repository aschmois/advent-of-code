...
async function main() {
    ...
    const minute = Object.keys(dates).reduce((prev, curr) => {
        const numberOfTimesAsleep = dates[curr][lazyGuard.guardId]
        if (!prev.asleep || prev.asleep < numberOfTimesAsleep) {
            return { asleep: numberOfTimesAsleep, minute: curr }
        } else {
            return prev
        }
    }, {})
    console.log('lazy guard minute:', minute)
    console.log(parseInt(lazyGuard.guardId) * parseInt(minute.minute))
}
